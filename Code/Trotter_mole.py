...
Exact_error = []
Trotter_error = []
with open("Data/Trotter_mole.txt","w") as File1:
    for evolution_time in T_list:
        # for num_steps in n_list:   
            # tmp = 4 * sqrt(num_steps)
            num_steps = 2 * evolution_time
            ...
            if True:
                ...
                for i_step in range(num_steps):
                    ...
                    h_ins_circuit.append(sub_circuit(h_ins_jw, trotter_order, trotter_term_ordering, time_for_single_trotter,qubits))
                    asp_sim_result = simulator.simulate(h_ins_circuit, initial_state = asp_wf_sim_curr).final_state_vector
                    ...
                    if i_step % 100 == 0:
                        print("  {:>.3f}".format(num_steps)," {:.3f}".format(td_hamiltonian_weight),
                        "   {:.10f}".format(e_asp),
